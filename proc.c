...
static struct rbtree *proctree = &rbTree;

// Latency must be multiples of min_granularity
static int latency = NPROC / 2; // Target scheduler latency
static int min_granularity = 2; // Minimum granularity
...
// Make sure the nice_value is capped at a maximum of 30.
int
compute_weight(int nice_value)
{
}
...
// Ensure that the nice_value is within valid bounds (0 to 30).
int
setnice(int pid, int nice_value)
{
}
...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  // Initialize CFS members of the process. Use compute_weight to calculate the weight of the process
  p->vruntime = 0;
  p->curr_runtime = 0;
  p->max_exec_time = 0;
  p->nice_value = 0;
